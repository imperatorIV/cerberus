...
if __name__ == "__main__":
    ...
    for file_name in tqdm([f for f in os.listdir(args.caption_path) if f.endswith('.json')], desc="Processing JSON Files"):
        ...
        for img_key, img_data in captions_data.items():
            ...
            cumulative_similarity_score += positive_similarity_score

            total_images += 1

            # Determine if the generated caption is more similar to the positive caption
            if positive_scores['rougeL'].fmeasure > negative_scores['rougeL'].fmeasure:
                num_correct_identifications += 1

            total_images_evaluated += 1

        # Calculate average similarity score and accuracy for the current JSON file
        average_similarity_score = cumulative_similarity_score / total_images if total_images != 0 else 0
        accuracy = (num_correct_identifications / total_images if total_images != 0 else 0) * 100

        # Store the results for the current JSON file
        overall_results[file_name.split('.')[0]] = {
        ...
        # # Store the accuracy in a dictionary, using a meaningful key
        ...
        # results_filename = "accuracy_results.json"

        # # Write the JSON string to a file
        # with open(results_filename, "w") as outfile:
        #     outfile.write(json_results)

        # print(f"Accuracy results written to {results_filename}")

    # Print scores in the desired format
    #print(json.dumps(scores))
        # Write the overall results to JSON files
    with open("similarity_scores.json", "w") as sim_file, open("accuracy_results.json", "w") as acc_file:
        similarity_scores = {key: value['average_similarity_score'] for key, value in overall_results.items()}
        json.dump(similarity_scores, sim_file)
        accuracies = {key: value['accuracy'] for key, value in overall_results.items()}
        json.dump(accuracies, acc_file)

    print("Results have been written to similarity_scores.json and accuracy_results.json")
